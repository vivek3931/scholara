                    ...
                    {isLoading ? (
                        ...
                        subjects.slice(0, 12).map((subject: any, i: number) => (
                            <Link href={`/browse?subject=${subject.value}`} key={i} className="group">
                                <Card className="bg-card border-border hover:border-primary/30 hover:bg-accent/50 transition-all duration-300 h-full backdrop-blur-sm group-hover:-translate-y-1 group-hover:shadow-lg overflow-hidden relative shadow-sm">
                                    <div className="absolute inset-0 bg-gradient-to-br from-primary/0 to-primary/0 group-hover:from-primary/5 group-hover:to-orange-500/5 transition-all duration-500" />
